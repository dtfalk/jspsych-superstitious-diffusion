...
// Build main (actual) stimuli list from the obfuscated STIMULUS_MAP,
// then split into balanced blocks (half of each letter per block).
let actual_stimuli = [];
if (typeof STIMULUS_MAP !== 'undefined') {
  // Group actual stimuli by letter so we can split each group in half
  const byLetter = {};
  ...
  // Shuffle within each letter group, then split each in half across blocks
  const block1 = [];
  const block2 = [];
  Object.keys(byLetter).forEach(letter => {
    ...
    block1.push(...items.slice(0, half));
    block2.push(...items.slice(half));
  });

  // Combine for the full list (used elsewhere if needed)
  actual_stimuli = [...block1, ...block2];
  // Store block splits for timeline variables
  var actual_stimuli_block_1 = block1;
  var actual_stimuli_block_2 = block2;

} else {
  // Fallback for local dev without obfuscation
  const _byLetter = {};
  CONFIG.targetLetters.forEach(condition => {
    _byLetter[condition] = [];
    for (let i = 0; i < CONFIG.main.nPerCondition; i++) {
      _byLetter[condition].push({
        stimulus: [`assets/stimuli/actual-stimuli/${condition}/${condition}_${String(i)}.mp4`]
      });
    }
  });
  ...
  const _block1 = [];
  const _block2 = [];
  Object.keys(_byLetter).forEach(letter => {
    const items = _byLetter[letter];
    for (let i = items.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [items[i], items[j]] = [items[j], items[i]];
    }
    const half = Math.ceil(items.length / 2);
    _block1.push(...items.slice(0, half));
    _block2.push(...items.slice(half));
  });

  actual_stimuli = [..._block1, ..._block2];
  var actual_stimuli_block_1 = _block1;
  var actual_stimuli_block_2 = _block2;
}
...
// Block 1 and block 2 are already built above with balanced letter distribution.
// Each block contains half of each letter's stimuli.
...
const actual_break = {
  ...
  on_start: trial => {
    trial.screen_presented_time = performance.now();
  },
  on_finish: data => {
