...
// Build practice stimuli list from the obfuscated STIMULUS_MAP.
// Each entry's metadata is decoded to filter for practice trials only.
...
if (typeof STIMULUS_MAP !== 'undefined') {
  Object.keys(STIMULUS_MAP).forEach(hexId => {
    ...
    if (meta && meta.trialType === 'practice') {
      practice_stimuli.push({
        ...
        _hexId: hexId  // stored for metadata lookup in on_finish
      });
    }
  });
} else {
  // Fallback for local dev without obfuscation (stimulus-map.js not loaded)
  CONFIG.targetLetters.forEach(condition => {
    for (let i = CONFIG.practice.stimulusStartIndex; i < CONFIG.practice.stimulusStartIndex + CONFIG.practice.nPerCondition; i++) {
      practice_stimuli.push({
        stimulus: [`assets/stimuli/practice-stimuli/${condition}/${condition}_${String(i)}.mp4`]
      });
    }
  });
}
