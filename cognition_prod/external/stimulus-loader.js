...
function loadStimuli() {
  ...
  // Apply limits if set (matches old config style)
  var practiceIndices = indices.practice;
  var actualIndices = indices.actual;
  
  // Check nPractice/nActual (PROD style) or practiceLimit/actualLimit (DEV style)
  var pLimit = CONFIG.nPractice || CONFIG.practiceLimit || 0;
  var aLimit = CONFIG.nActual || CONFIG.actualLimit || 0;
  
  if (pLimit > 0) {
    practiceIndices = practiceIndices.slice(0, pLimit);
  }
  if (aLimit > 0) {
    actualIndices = actualIndices.slice(0, aLimit);
  }
  
  // Clear arrays by mutating (not reassigning) to preserve references
...
function buildActualStimuliWithBlockSplit(condition, pairIndices) {
  // Clear by mutating to preserve references
  ...
  // Combined array for preloading - mutate to preserve reference
